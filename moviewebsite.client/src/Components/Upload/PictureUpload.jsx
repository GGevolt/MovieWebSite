...
function PictureUpload({id, onSuccess}){
    ...
    const handleUploadFile =()=>{
        if(imageFile && id >0){
            ...
            .catch(error => {
                console.error('There has been a problem with the film picture submission:', error);
            });
        }
    }
    return (
                        ...
                        {isDragging? (<span className='select'>Drop Image Here</span>):
                            image.length ===0?
                                (<>
                                    ...
                                    <span className='select' role='button' onClick={selectFile}>Browse file</span>
                                </>):
                                (<Container>
                                    <div className='image'>
                                        <span className='delete' onClick={handleDelete}>&times;</span>
                                        <Image src={image[0].url} alt={image[0].name}/>
                                    </div>
                                </Container>)
                        }
                        <input type='file' className='file' name='file' ref={fileInputRef} onChange={handleFileSelect} />
                    </div>
