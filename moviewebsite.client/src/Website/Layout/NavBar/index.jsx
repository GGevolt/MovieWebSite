...
function NavBar() {
    ...
    >
      <div
        className={`Category-Container ${
          isMobile ? "mobile-category-container" : ""
        }`}
      >
  ...
  const renderUserMenu = (isMobile = false) => (
    <NavDropdown
      title={
        <>
          <User size={20} /> <span className="username-text">{userName}</span>{" "}
      ...
      <>
        ...
        <Navbar.Collapse className="justify-content-end">
          <Row className="navbar-right">
            ...
            <Col xs="auto">{renderUserMenu()}</Col>
            <Col xs="auto">
  ...
  const renderAuthLinks = () => {
    let roleHandler;
    if (
      roles.includes("UserT0") ||
      roles.includes("UserT1") ||
      roles.includes("UserT2")
    ) {
